...
module.exports = {
  ...
  insertSetAndOffer: function( setData, offerData, bookTypes, keepPhotos, req, db ){
    ...
    let offerDataFix = offerData;
    offerDataFix.description = offerDataFix.description.replace(/[\r\n]+/gm, "");
  ...
  modifyOffer: function( req, res, db ){
          ...
          if( typeof req.body.password == 'undefined' ){
            ...
            sqlChangeUserData =`UPDATE users
                                ...
                                WHERE username = '${userData.username}' AND code = '${req.body.random}' AND id = '${req.body.id}'`;
          }
                      
          else{
            sendEmail = true;
            sqlChangeUserData =`UPDATE users
                                SET code = '${userData.code}', email = '${userData.email}', num = '${userData.num}'
                                WHERE username = '${userData.username} AND code='${req.body.random}' AND id = '${req.body.id}`;
          }
          ...
          let queryChangeUserData = db.query( sqlChangeUserData, ( err, changeResult ) => {
            ...
            sqlChangeOfferData =  `UPDATE offers
                                   ...
                                   price_from='${req.body.price_from}' WHERE id = '${req.body.offerId}'`;

          })
            
          if( sqlChangeOfferData !== "undefined")
