    ...
    */
    ...
    let queryExistingUserOut = db.query(sqlExistingUser, (err, foundUsers) => {
      ...
      if (foundUsers.length == 0) {
        ...
        res.render('index', { hint: `Witaj ${userData.username} ! Twoje hasło do zmian w ogłoszeniu: <p class="modalPass">${userData.code}</p>`, type: 'newUser' });
        ...
        if (userData.code === entryCode) {
          ...
          queryController.insertSetAndOffer(setData, offerData, bookTypes, keepPhotos, req, db)
          res.render('index', { hint: `Witaj ${userData.username} ! Pomyślnie dodałeś ofertę.`, type: "correctCode" });
